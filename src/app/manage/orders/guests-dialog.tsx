...
export default function GuestsDialog({ onChoose }: { onChoose: (guest: GuestItem) => void }) {
  ...
  const listGuestsQuery = useGetGuestListQuery({ fromDate, toDate });
  const data: GetListGuestsResType["data"] = listGuestsQuery.data?.payload.data || [];
  ...
  const choose = (guest: GuestItem) => {
    onChoose(guest); // chuyển dữ liệu từ child lên parent thông qua hàm onChoose (props)
    setOpen(false);
  };
